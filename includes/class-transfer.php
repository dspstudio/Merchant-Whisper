...
class MW_Sales_Toast_Transfer {
	...
	public static function handle_export() {
		if ( ! current_user_can( MW_Sales_Toast_Settings::capability() ) ) {
			wp_die( esc_html__( 'You do not have permission to export Sales Toast settings.', 'mw-sales-toast' ), '', array( 'response' => 403 ) );
		}
	...
	public static function handle_import() {
		...
		if ( ! current_user_can( MW_Sales_Toast_Settings::capability() ) ) {
			wp_die( esc_html__( 'You do not have permission to import Sales Toast settings.', 'mw-sales-toast' ), '', array( 'response' => 403 ) );
		}
		check_admin_referer( self::NONCE, 'mw_st_transfer_nonce' );
	...
	public static function render_controls( $kind ) {
		...
		$confirm  = ( 'theme' === $kind )
			...
			: __( 'Replace the current Sales Toast settings with this file? Product and category IDs are site-specific.', 'mw-sales-toast' );
		?>
